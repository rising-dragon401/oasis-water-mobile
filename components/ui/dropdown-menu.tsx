...
>(({ className, inset, children, ...props }, ref) => {
	...
	const getIcon = (open: boolean): JSX.Element => {
		if (Platform.OS === "web") {
			return <Octicons name="chevron-right" size={14} color="black" />;
		} else if (open) {
			return <Octicons name="chevron-up" size={14} color="black" />;
		} else {
			return <Octicons name="chevron-down" size={14} color="black" />;
		}
	};

	// Then use it like this in your component
	const Icon = getIcon(open);
		...
		<DropdownMenuPrimitive.Portal>
			...
			>
				<DropdownMenuPrimitive.Content
					ref={ref}
					className={cn(
						"z-50 min-w-[8rem] overflow-hidden rounded-xl bg-card border border-border p-1 shadow-md shadow-foreground/5 web:data-[side=bottom]:slide-in-from-top-2 web:data-[side=left]:slide-in-from-right-2 web:data-[side=right]:slide-in-from-left-2 web:data-[side=top]:slide-in-from-bottom-2",
						open
							? "web:animate-in web:fade-in-0 web:zoom-in-95"
